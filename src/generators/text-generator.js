...
역할:
- 수집된 뉴스와 증시 데이터를 하나의 주제로 묶어 7장짜리 카드뉴스 세트를 만듭니다 (커버1 + 콘텐츠5 + CTA1)
...
5장 - 뉴스4: 오늘 테마의 네 번째 각도/심화 정보
6장 - 뉴스5: 오늘 테마의 다섯 번째 각도/전망·요약
7장 - CTA: 오늘의 요약 + 앱 다운로드 유도

⚠️ 핵심 규칙: 슬라이드 2·3·4·5·6은 반드시 오늘의 테마 하나로만 통일. 다른 카테고리 뉴스 혼용 절대 금지.
...
8-1. imageKeywordKo: 네이버 이미지 검색에 사용할 한국어 핵심 키워드 1~3단어. 인물이 있으면 이름 우선 (예: "이재명 대통령", "코스피 주식", "한국은행 금리", "삼성전자 주가", "시니어 건강 운동"). 최대한 뉴스 핵심 키워드로.
  ...
  "slides": [
    {
      ...
      "imageKeywordKo": "한국어 핵심 키워드 (예: 이재명 대통령)"
    },
    { "slideNum": 3, "category": "...", "title": "...", "body": "...", "keyPoints": ["...", "..."], "source": "...", "imageKeyword": "...", "imageKeywordKo": "..." },
    { "slideNum": 4, "category": "...", "title": "...", "body": "...", "keyPoints": ["...", "..."], "source": "...", "imageKeyword": "...", "imageKeywordKo": "..." },
    { "slideNum": 5, "category": "...", "title": "...", "body": "...", "keyPoints": ["...", "..."], "source": "...", "imageKeyword": "...", "imageKeywordKo": "..." },
...
async function generateCardNewsText(newsData) {
  ...
  const THEME_GUIDE = {
    '주식':    '슬라이드2·3·4·5·6 모두 주식/증시/산업 소식으로만 구성. 종목, 지수, 업종 동향 중심.',
    '재테크':  '슬라이드2·3·4·5·6 모두 재테크/절약/자산관리/부동산 정보로만 구성. 50~60대가 실행 가능한 구체적 팁 포함.',
    '건강':    '슬라이드2·3·4·5·6 모두 건강/의료/식단/운동 정보로만 구성. 50~60대 시니어 맞춤 실용 정보.',
    '최신뉴스': '슬라이드2·3·4·5·6 모두 정치/사회/국제 최신 이슈로만 구성. 50~60대 관심사(복지, 정책, 사회 이슈) 중심.'
  };
...
⚠️ slides 배열은 반드시 정확히 5개 (slideNum 2, 3, 4, 5, 6). 5개보다 적거나 많으면 안 됩니다.
총 7장 구성: 커버(1장) + 콘텐츠 슬라이드(5장) + CTA(1장) = 7장
